...
class PNG:
   ...
   # @debug_timer
    def read_chunks(self):
        ...
        if idat_data: # check that there is image data
            ...
            for y in range(self.height):
                ...
                # Filter type 3 - average [(filtered byte + ((left Byte + above Byte) // 2)) % 256]
                    ...
                    print(f'No valid filter method for scanline: {y}') 
    ...
    def save_rgb(self, file_name: str, rgb_option: int):
        ...
        if rgb_option == 1: # To store only Red channels
            for y in range(self.height):
                ...
                for x in range(self.width):
                    raw_data.extend([self.img[y][x][0], 0, 0]) # Only get the R value from img, let G & B be 0

        elif rgb_option == 2: # To store only Green channels
            for y in range(self.height):
                raw_data.append(0)  # Add filter type byte (0 for no filter)
                for x in range(self.width):
                    raw_data.extend([0, self.img[y][x][1], 0]) # Only get the G value from img, let R & B be 0

        elif rgb_option == 3: # To store only Blue channels
            for y in range(self.height):
                raw_data.append(0)  # Add filter type byte (0 for no filter)
                for x in range(self.width):
                    raw_data.extend([0, 0, self.img[y][x][2]]) # Only get the B value from img, let R & G be 0
