...
class PNG:
    ...
    def read_chunks(self):
        offset = 8  # Begin chunk iteration at the start of IHDR chunk 
        idat_data = b''
        ...
        while offset < len(self.data):
            ...
            if chunk_type == b'IDAT':
                idat_data += chunk_data # concatonate all the IDAT data together
        ...
        if idat_data: # ensure image data exists
            ...
            print('no image data in the IDAT')

    def save_rgb(self, file_name: str, rgb_option: int):
        raw_data = [] # Create raw data list to store the specific RGB channels

        if rgb_option == 1: # To store only Red channels
            for y in range(self.height):
                raw_data.append(0)  # Append the filter type byte (0 for no filter)
                for x in range(self.width):
                    # Only append the R value from img, append G & B as 0
                    raw_data.append(self.img[y][x][0]) 
                    raw_data.append(0) 
                    raw_data.append(0)

        elif rgb_option == 2: # To store only Green channels
            for y in range(self.height):
                raw_data.append(0)  # Append filter type byte (0 for no filter)
                for x in range(self.width):
                    # Only append the G value from img, append R & B as 0
                    raw_data.append(0)
                    raw_data.append(self.img[y][x][1]) 
                    raw_data.append(0)

        elif rgb_option == 3: # To store only Blue channels
            for y in range(self.height):
                raw_data.append(0)  # Append filter type byte (0 for no filter)
                for x in range(self.width):
                    # Only append the B value from img, append R & G as 0
                    raw_data.append(0)
                    raw_data.append(0)
                    raw_data.append(self.img[y][x][2]) 

        rgb_compressed_idat_data = zlib.compress(bytes(raw_data)) # Cast raw_data list to bytes and compress it
    
        with open(file_name, 'wb') as file:
            # Write the PNG signature
            file.write(b'\x89PNG\r\n\x1a\n')

            # Write the IHDR chunk (will be the same as original data, will always be the first chunk)
            file.write(self.data[8:33])
            ...
            # Write the IEND chunk (will be the same as original data, and will always be the last chunk)
            file.write(self.data[-12:])
